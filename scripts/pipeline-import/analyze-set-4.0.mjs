// Set 4.0 Mix — focused analyzer.
import fs from 'node:fs';
import path from 'node:path';
import { execSync } from 'node:child_process';

const TAB = 'Set 4.0 Mix';
const SLUG = 'set-4.0-mix';
...
if (noId.length) {
  console.log('First few rows missing numeric ID:');
  for (const r of noId.slice(0,3)) console.log('  ',JSON.stringify({ID:r.ID,Title:r.Title,SKU:r.SKU,QTY:r.QTY,_purchased:r._purchased}));
}
...
const enriched = rows.map(r => {
  ...
  return {
    ...
    pipeline_status_sheet: r.PipelineStatus, purchased: r._purchased,
    db_exists: !!m.exists, db_original_id: m.original_id || null,
    db_name: m.name || null, db_pipeline_status: m.pipeline_status || null,
    db_shortlisted: m.shortlisted || false, existing_po_lines: m.poCount || 0,
  };
});
...
let qtySum = 0, qtyMissing = 0, cogsMissing = 0, transportMissing = 0, lineCostSum = 0;
for (const r of [...importableOrdered, ...importableNew]) {
  const q = num(r.qty);
  if (q == null) qtyMissing++; else if (r.purchased) qtySum += Math.round(q);
  ...
  if (r.purchased) {
    ...
    lineCostSum += unit * Math.round(q??0);
  }
}
console.log(`\nQty sum (ordered only):     ${qtySum}`);
console.log(`Estimated total cost USD:   ${lineCostSum.toFixed(2)}`);
