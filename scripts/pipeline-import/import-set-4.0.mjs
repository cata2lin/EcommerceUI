// Import Set 4.0 Mix — same behavior as Set 3.
...
const PO_TITLE = 'Pipeline Set 4.0 Mix';
const PO_NOTES = 'Imported from Grandia Pipeline sheet on 2026-04-27. 50 ordered + 9 supplier-skipped.';
...
const rawLines = importable.map(r => {
  ...
  return {
    ...
    purchased: r.purchased, raw_qty: qtySheet,
  };
});
...
for (const l of rawLines) {
  ...
  if (l.purchased && !existing.purchased) byProduct.set(l.product_id, { ...l, _rows: existing._rows });
}
...
const plannedLines = [...byProduct.values()].map(l => ({
  ...l, line_cost_usd: +(l.unit_cost_usd * l.quantity).toFixed(2),
}));
