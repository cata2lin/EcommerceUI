/**
 * ApprovedItems — Purchase Orders page showing approved products ready for procurement.
...
export default function ApprovedItems() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.error('Approved items load failed:', err);
    } finally {
      setLoading(false);
    }
  }, [page, sortBy, sortDir, appliedFilters]);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `approved-items-purchase-orders.xlsx`;
      a.click();
    ...
    <div className="animate-fade-in">
      <div className="page-header">
        <div className="flex items-center gap-4">
          <h1>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ marginRight: '12px', color: 'var(--color-primary)' }}>
          ...
          <span className="text-sm" style={{ opacity: 0.7 }}>{total} products ready for procurement</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button className="btn btn-primary btn-sm" disabled={products.length === 0}>
